...
void construcao_gulosa(int m, int n, int* c, int* c_aux, int* Ind, int* y, int* z, int* d, int** A, float demand) {
	...
	//Ordenaçao dos vetores de custo de abertura das facilidades em ordem crescente 
	for (int i = 0; i < n; i++) {
		for (int j = i + 1; j < n; j++) {
			if (c[j] < c[i]) {
				aux = c[j];
	...
	//Implementaçao do Vetor de abertuta de facilidades (0-1) 
	for (int i = 0; i < n; i++) {
		y[i] = 0;
		//printf(" %d", y[i]);		
	}
	...
	while (demand >= dem) {
		...
		//Declaraçao variável de Funçao Objetivo
		...
		for (int i = 0; i < m; i++) {
			...
			if (soma > 0) z[i] = 1; //Caso o nó i seja coberto por alguma facilidade, y[i] será trocado para 1
			//printf(" %d", z[i]);
		}
